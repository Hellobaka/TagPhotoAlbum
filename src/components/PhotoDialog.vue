<template>
  ...
  <Transition name="dialog-fade">
    <div
      v-if="showUnsavedTagDialog"
      class="dialog-overlay"
      @click="showUnsavedTagDialog = false"
    >
      <Transition name="dialog-scale">
        <div class="confirm-dialog" @click.stop>
          ...
          <div class="dialog-actions">
            <md-text-button
              @click="cancelUnsavedTag"
              style="padding-left: 15px; padding-right: 15px"
            >
              取消
            </md-text-button>
            <md-text-button
              @click="saveWithoutUnsavedTag"
              style="padding-left: 15px; padding-right: 15px"
            >
              不添加，直接保存
            </md-text-button>
            <md-text-button
              @click="addUnsavedTagAndSave"
              style="padding-left: 15px; padding-right: 15px"
            >
              添加并保存
            </md-text-button>
...
const savePhotoInfo = async () => {
  ...
  if (
    newTag.value.trim() &&
    !editablePhoto.value.tags.includes(newTag.value.trim())
  ) {
