  ...
  <div class="photo-editor-container">
    ...
    <div class="left-panel">
      <div class="photo-container" @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave">
        <img class="photo" :src="currentImageUrl" :alt="photo.title || '图片'" />
        <div class="original-image-button"
          :class="{ 'show': showOriginalButton && photo?.compressedFilePath && !isShowingOriginal }">
          <md-filled-tonal-button @click="toggleOriginalImage" style="padding-left: 15px; padding-right: 15px;">
    ...
    <div class="right-panel">
      ...
      <div class="collapsible-section details-section" v-if="photo">
        <div class="section-header" @click="toggleDetailsVisibility">
          <h3 class="md-typescale-title-medium">
            <span class="material-symbols-outlined collapse-icon"
              :class="{ 'expanded': isDetailsVisible }">expand_more</span>
            图片详情
          </h3>
        </div>
        ...
        <div class="section-content" v-show="isDetailsVisible">
          ...
          <div class="detail-item" v-if="photo.fileSizeKB">
            ...
            <div class="detail-value">{{ formatFileSize(photo.fileSizeKB) }}</div>
          </div>
          ...
          <div class="exif-section" v-if="filteredExifData.length > 0">
            <div class="exif-header">
              ...
              <md-switch class="dense" :selected="showExifDetails" @change="showExifDetails = !showExifDetails" />
            </div>
            <div v-if="showExifDetails" style="display: flex; align-items: center;">
              <md-switch class="dense" :selected="hideUncommonExif" @change="hideUncommonExif = !hideUncommonExif" />
              <h4 class="md-typescale-body-medium" style="margin-left: 10px;">隐藏非常用 Exif 值</h4>
            </div>
            <div v-if="showExifDetails && !hideUncommonExif" style="display: flex; align-items: center; margin-bottom: 8px;">
              <md-switch class="dense" :selected="hideUndenfiedExif" @change="hideUndenfiedExif = !hideUndenfiedExif" />
              <h4 class="md-typescale-body-medium" style="margin-left: 10px;">隐藏未定义值</h4>
            </div>
            <div v-show="showExifDetails" v-for="exifItem in filteredExifData" :key="exifItem.key" class="detail-item">
              <div class="detail-label">
                <span class="material-symbols-outlined">{{ getExifIcon(exifItem.key) }}</span>
      ...
      <div class="collapsible-section info-section">
        <div class="section-header" @click="toggleInfoVisibility">
          <h3 class="md-typescale-title-medium">
            <span class="material-symbols-outlined collapse-icon"
              :class="{ 'expanded': isInfoVisible }">expand_more</span>
            编辑信息
          </h3>
        </div>

        <div class="section-content" v-show="isInfoVisible">
          <md-outlined-text-field :value="editablePhoto.title" @input="e => $emit('update:title', e.target.value)"
            label="标题" class="info-field" />

          <md-outlined-text-field :value="editablePhoto.description"
            @input="e => $emit('update:description', e.target.value)" label="描述" type="textarea" rows="3"
            class="info-field" />
          ...
          <div class="info-grid">
            <div class="folder-field">
              <md-outlined-text-field :value="editablePhoto.folder" @input="handleFolderInput"
                @focus="showFolderSuggestions = true" @blur="handleFolderBlur" label="文件夹" class="info-field"
                ref="folderInput" />
              <div v-if="showFolderSuggestions && filteredFolders.length > 0" class="folder-suggestions"
                :class="{ 'fade-out': isFolderSuggestionsClosing }" :style="getFolderSuggestionsStyle()">
                <div v-for="folder in filteredFolders" :key="folder" class="suggestion-item"
            ...
            <md-outlined-text-field :value="editablePhoto.location"
              @input="e => $emit('update:location', e.target.value)" label="地点" class="info-field" />
          </div>
        </div>
      </div>
    </div>
  ...
  if (typeof value === 'object' && !exifObjectKey.includes(key)) {
    ...
    if (Array.isArray(value)) {
      return value.map(item => {
        if (typeof item === 'object' && item.numerator !== undefined && item.denominator !== undefined) {
  ...
  switch (key) {
    case 'ExposureTime':
      if (typeof value === 'object' && value.numerator !== undefined && value.denominator !== undefined) {
        ...
        return exposure >= 1 ? `${exposure}秒` : `1/${Math.round(1 / exposure)}秒`
      }
      break
    ...
    case 'ExposureBiasValue':
      if (typeof value === 'object' && value.numerator !== undefined && value.denominator !== undefined) {
        return `${value.numerator > 0 ? '+' : (value.numerator == 0 ? '' : '-')}${(value.numerator / value.denominator).toFixed(1)} EV`
      }
      break
    ...
    case 'GPSLongitude':
      if (Array.isArray(value) && value.length === 3) {
        ...
        return `${degrees.toFixed(6)}° ${minutes.toFixed(0)}' ${seconds.toFixed(2)}\"`
      }
      break
...
const filteredExifData = computed(() => {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (value !== null && value !== undefined && value !== '') {
        if (exifObjectKey.includes(key)) {
          if (!hideUncommonExif.value || (hideUncommonExif.value && getExifIsCommon(fullKey))){
            exifItems.push({
              key: key,
        ...
        } else if (!hideUncommonExif.value || (hideUncommonExif.value && getExifIsCommon(fullKey))) {
          if (hideUndenfiedExif.value && !(commonExifLabelMap[fullKey] || uncommonExifLabelMap[fullKey] || commonExifLabelMap[key] || uncommonExifLabelMap[key])) {
            continue;
          }
          exifItems.push({
            key: fullKey,
  ...
  // 按重要性排序
  const priorityKeys = [
    'Make', 'Model', 'LensMake', 'LensModel', 'DateTimeOriginal', 'ExposureTime', 'ExposureBiasValue', 'FNumber',
    'ISOSpeedRatings', 'FocalLength', 'WhiteBalance', 'Flash',
  ...
  return exifItems.sort((a, b) => {
    ...
    if (aIndex !== -1 && bIndex !== -1) {
      ...
      return 1
    } else if ((commonExifLabelMap[a.key] || uncommonExifLabelMap[a.key]) && (!commonExifLabelMap[b.key] && !uncommonExifLabelMap[b.key])) {
      return -1
    } else if ((commonExifLabelMap[b.key] || uncommonExifLabelMap[b.key]) && (!commonExifLabelMap[a.key] && !uncommonExifLabelMap[a.key])) {
      return 1
    } else {
